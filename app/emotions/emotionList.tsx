...
export default function EmotionList() {
  ...
  const renderMoodIcon = (mood: Mood) => {
    switch (mood) {
      case Mood.HAPPY:
        return (
          <RiEmotionHappyLine className="text-2xl text-blue-500 mx-auto" />
        );
      case Mood.NORMAL:
        return (
          <RiEmotionNormalLine className="text-2xl text-indigo-500 mx-auto" />
        );
      case Mood.SAD:
        return (
          <RiEmotionUnhappyLine className="text-2xl text-pink-500 mx-auto" />
        );
      default:
        return null;
    }
  ...
  return feelings.length === 0 ? (
    <p className="text-center text-gray-500">
      No feelings recorded for this month.
    </p>
  ) : (
